...
from .api import AppInfo, FlatseaAPI, Page
...
log = logging.getLogger("flatsea.window")
...
class FlatseaWindow(Adw.ApplicationWindow):
    def __init__(self, app: Adw.Application, api: FlatseaAPI):
        super().__init__(application=app, title="Flatsea", default_width=1100, default_height=720)
        self.api = api
    ...
    def _build_browse_page(self) -> Adw.NavigationPage:
        ...
        side_tb.add_top_bar(Adw.HeaderBar(title_widget=Adw.WindowTitle(title="Flatsea")))
        ...
        self.stack.add_named(Adw.StatusPage(icon_name="network-offline-symbolic", title="Can't reach the Flatsea server",
    ...
    def _app_menu(self):
        ...
        menu.append("About Flatsea", "app.about")
        return menu
    ...
    def reload(self, append: bool = False) -> None:
        ...
        page_no = self._page
        q, cat, risk, sort = self._query, self._category, self._risk, self._sort

        def _done(page: Page):
            if (q, cat, risk, sort) != (self._query, self._category, self._risk, self._sort):
                return  # stale
            if not append:
                _clear(self.grid)
        ...
        run_async(lambda: self.api.list_apps(q=q, category=cat, risk=risk, sort=sort, page=page_no), _done, _err)

    def refresh_installed(self) -> None:
        def _done(ids):
            self.installed = ids
            for page in self._pages.values():
                page.set_installed(page.app.app_id in ids)

        run_async(fp.installed_ids, _done, lambda e: None)
    ...
    def open_app(self, app_id: str) -> None:
        if app_id in self._pages:
            self.nav.push(self._pages[app_id])
            return
        self.status.set_text(f"Loading {app_id}…")

        def _done(app: AppInfo):
            page = AppPage(app, app.app_id in self.installed, self.install_app, self.uninstall_app, self.launch_app)
    ...
    def install_app(self, app: AppInfo) -> None:
        page = self._pages.get(app.app_id)
        if page:
            page.set_busy(True)

        def _status(msg: str):
            GLib.idle_add(lambda: (self.status.set_text(msg), False)[1])
        ...
        def _done(outcome: pipeline.Outcome):
            ...
            if outcome.installed:
                ...
                self.toast(f"Did not install {app.name}.")
            else:
                self.toast(outcome.message or f"Install of {app.name} failed.", timeout=8)
        ...
        run_async(lambda: pipeline.install(app, self.api, self.confirmer, self.decisions, _status), _done, _err)
    ...
    def uninstall_app(self, app: AppInfo) -> None:
        ...
        run_async(lambda: fp.uninstall(app.app_id), _done, lambda e: self.toast(str(e)[:160]))
