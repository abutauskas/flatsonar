"""Entry point: ``python -m flatsonar`` or the ``flatsonar`` script."""
...
def main(argv: list[str] | None = None) -> int:
    logging.basicConfig(level=logging.DEBUG if os.environ.get("FLATSONAR_DEBUG") else logging.INFO,
    ...
    try:
        ...
        print(f"Flatsonar needs GTK 4 and libadwaita with Python bindings: {exc}\n"
    ...
    class FlatsonarApp(Adw.Application):
        def __init__(self):
            ...
            # The website's teal accent and palette come from widgets.CSS: libadwaita has no
            # API for an app to pick its accent, only the --accent-* CSS variables.
        ...
        def do_activate(self):
            if self.window is None:
                self.window = FlatsonarWindow(self, FlatsonarAPI(DEFAULT_API))
            self.window.present()
    ...
    return FlatsonarApp().run(argv if argv is not None else sys.argv)
